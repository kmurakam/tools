...
def make_num_img(path,start_num=0,nfiles=10,sub_nfiles=2,hight=100,width=200,extension="jpeg"):
    """path:作成パス, start_num:開始番号, nfiles:作成画像総数, sub_nfiles:サブフォルダ毎の画像数, hight:画像縦幅, width:画像横幅"""
    sub_fol_name=f"{str(start_num)}-{str(start_num+sub_nfiles-1)}" #サブフォルダ名
    os.makedirs(f"{path}\{sub_fol_name}",exist_ok=True)
    for i in range(nfiles):
        ...
        cv2.imwrite(f"{path}\{sub_fol_name}\{str(i+start_num)}.{extension}",img)

        if (i+1) % sub_nfiles == 0 and (i+1) != nfiles: #出力した画像数がサブフォルダ毎の画像数の倍数かつ一番最後でない場合
            sub_fol_name=f"{str(i+start_num+1)}-{str(i+start_num+sub_nfiles)}"
            os.makedirs(f"{path}\{sub_fol_name}",exist_ok=True)
